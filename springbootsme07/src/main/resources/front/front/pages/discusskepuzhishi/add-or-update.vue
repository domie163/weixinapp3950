	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`discusskepuzhishi`, this.ruleForm.id);
...
//跨表计算判断
				...
				if(crossrefid && crossuserid) {
					...
					let res = await this.$api.list(`discusskepuzhishi`, params);
                ...
                //跨表计算
						if(this.ruleForm.id){
							await this.$api.update(`discusskepuzhishi`, this.ruleForm);
						}else{
							await this.$api.add(`discusskepuzhishi`, this.ruleForm);
						}
						uni.setStorageSync('discusskepuzhishiCleanType',true);
